class Voter:
    def __init__(self,ID=None,name='',fhname='',age=0,address='',province='',vdicty={},tlisty=[]):
        self._id = ID
        self._name = name
        self._fhname = fhname
        self._age = age
        self._adres = address
        self._prov = province
        self._vdicty,self.tlisty = vdicty,tlisty
        
        if self._id != None: 
            self._vdicty = {'ID':str(self._id),'Name':self._name,'Father/Husband name':self._fhname,'Age':self._age,'Address':self._adres,'Province':self._prov}           
            self.tlisty.append(self._vdicty)
            ally=[]
